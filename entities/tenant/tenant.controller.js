...
// ====== Branding: Tenant Signature ======

// GET presigned URL for signature upload
const getSignatureUploadUrl = asyncHandler(async (req, res) => {
  const { contentType, extension } = req.query;
  ...
  const tenantSegment = sanitizeSegment(tenantDoc.name || 'tenant', true);
  const filename = buildDatedFilename('signature', safeExt);

  try {
    const result = await generateUploadUrl({
      tenantSegment,
      contentType,
      pattern: 'tenant-signature',
      filename,
    });
    return res.status(200).json({ key: result.key, uploadUrl: result.uploadUrl });
  } catch (err) {
...
// PUT set/remove signature for current tenant
const setTenantSignature = asyncHandler(async (req, res) => {
  const { fileKey } = req.body; // string to set, or null to remove

  const tenant = await Tenant.findById(req.tenant);
  if (!tenant) return res.status(404).json({ message: 'Tenant not found' });

  const prevKey = tenant.signatureKey;

  if (fileKey === null) {
    // Remove signature
    tenant.signatureKey = null;
    tenant.signatureUrl = null;
    tenant.signatureUpdatedAt = new Date();
    await tenant.save();
    // Best-effort delete old
    if (prevKey) {
      try {
        await deleteObjectFromS3(prevKey);
      } catch (e) {
        /* non-blocking */
      }
    }
